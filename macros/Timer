//Fonte original: https://github.com/Kekilla0/Personal-Macros/blob/master/GM%20Macros/Timer.js
...
let times = {
  buttons : [
	["30 seg", ()=> { seconds_selected = seconds_left = 30; update_dialog();}],
  ...
  content : `<div sytle="width:100%; text-align:center;"><h2> Escolhar um timer: </h2></div>`,
  title : `Timer`
};
...
async function update_dialog()
{
  ...
  function getContent()
  {
    ...
    if(seconds_left !== 0 )
    {
      ...
      if(dialog.data.content.includes(`Fim`))
      {
        return ``;
      }else{
        return `<div sytle="width:100%; text-align:center; color:red;"><h1>Fim</h1></div>`;
      }
    }
  }
  function getButtons()
  {
    return {
      Restart : { label : `Reiniciar`, callback : () => { seconds_left = seconds_selected; update_dialog(); }},
      Cancel : { label : `Cancelar`, callback : () => { clearInterval(interval); }}
    };
  }
  function update()
  {
